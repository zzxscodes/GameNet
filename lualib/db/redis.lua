...
local function connect(host, port, opts)
    ...
    port = tonumber(port)
    if not port then
        error("bad argument #2 port: number expected, got " .. type(port), 2)
    end

    if opts and type(opts) ~= "table" then
        error("bad argument #3 opts: nil or table expected, got " .. type(opts), 2)
    end
...
function _M.new(host, port, opts)
    ...
    return setmetatable({
        _sock = sock,
        _subscribed = false,
        _proxy = opts and opts.proxy,
        _n_channel = { unsubscribe = 0, punsubscribe = 0 },
    }, mt)
end
...
local function _read_reply(self, sock)
    ...
    if prefix == 36 then
        ...
        if size < 0 then return null end
...
local function _do_cmd(self, ...)
    ...
    if rawget(self, "_proxy") then return true end

    local res, err = _read_reply(self, sock)
    while _check_msg(self, res) do
        if not rawget(self, "_buffered_msg") then
